...
# Create directories
RUN mkdir -p /opt/krew /home/k8suser/.krew && \
    chown -R k8suser:k8suser /opt/krew /home/k8suser/.krew
...
RUN KUBECTX_VERSION=$(curl -s https://api.github.com/repos/ahmetb/kubectx/releases/latest | grep tag_name | cut -d '"' -f 4) && \
    curl -L "https://github.com/ahmetb/kubectx/releases/download/${KUBECTX_VERSION}/kubectx_${KUBECTX_VERSION#v}_linux_x86_64.tar.gz" | tar xz && \
    curl -L "https://github.com/ahmetb/kubectx/releases/download/${KUBECTX_VERSION}/kubens_${KUBECTX_VERSION#v}_linux_x86_64.tar.gz" | tar xz && \
...
RUN wget -qO - https://aquasecurity.github.io/trivy-repo/deb/public.key | apk add --allow-untrusted - && \
    echo "https://aquasecurity.github.io/trivy-repo/rpm/releases/" >> /etc/apk/repositories && \
    apk update && \
    # Alternative: Install trivy from GitHub releases since Alpine package may not be available
